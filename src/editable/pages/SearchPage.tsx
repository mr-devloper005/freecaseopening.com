...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  return (
    <Link href={href} className={`group block overflow-hidden rounded-[2rem] border border-[var(--editable-border)] bg-white shadow-[0_16px_38px_rgba(84,22,44,0.08)] transition hover:-translate-y-1 hover:shadow-[0_24px_58px_rgba(84,22,44,0.16)] ${strong ? 'md:col-span-2' : ''}`}>
      {image ? (
      ...
      <div className="p-5 sm:p-6">
        {!image ? <span className="rounded-full bg-[var(--slot4-page-text)] px-3 py-1 text-[11px] font-black uppercase tracking-[0.18em] text-white">{taskLabel}</span> : null}
        <h2 className="editable-display mt-4 line-clamp-3 text-2xl font-extrabold leading-[1.02] tracking-[-0.04em] text-[var(--slot4-page-text)]">{post.title}</h2>
...
export default async function SearchPage({ searchParams }: { searchParams?: Promise<{ q?: string; category?: string; task?: string; master?: string }> }) {
  ...
  const enabledTasks = SITE_CONFIG.tasks.filter((item) => item.enabled && !['listing', 'classified'].includes(item.key))
      ...
      <main className="min-h-screen bg-[var(--slot4-page-bg)] text-[var(--slot4-page-text)]">
        <section className="relative overflow-hidden bg-[linear-gradient(180deg,#fb5a72_0%,#fb6371_58%,#ff7c6c_100%)] text-white">
          ...
          <div className="mx-auto max-w-[var(--editable-container)] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
            <div className="grid gap-8 rounded-[2rem] bg-white/12 p-6 shadow-[0_18px_50px_rgba(154,3,30,0.18)] backdrop-blur md:grid-cols-[0.8fr_1.2fr] lg:p-8">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.28em] text-white/75">{pagesContent.search.hero.badge}</p>
                <h1 className="editable-display mt-5 text-4xl font-extrabold leading-[1.02] tracking-[-0.05em] sm:text-6xl">{pagesContent.search.hero.title}</h1>
                <p className="mt-6 max-w-xl text-base font-semibold leading-8 text-white/84">{pagesContent.search.hero.description}</p>
              </div>
              <form action="/search" className="self-end rounded-[2rem] bg-white p-4 text-[var(--slot4-page-text)] sm:p-5">
                ...
                <button className="mt-3 inline-flex h-12 w-full items-center justify-center rounded-2xl bg-[var(--slot4-page-text)] px-6 text-sm font-black uppercase tracking-[0.18em] text-white transition hover:-translate-y-0.5" type="submit">Search</button>
              </form>
            </div>
          </div>
        </section>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-12">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.24em] text-[var(--slot4-accent)]">{results.length} results</p>
              <h2 className="editable-display mt-2 text-3xl font-extrabold tracking-[-0.04em]">{query ? `Results for "${query}"` : pagesContent.search.resultsTitle}</h2>
            </div>
          ...
          {results.length ? (
            ...
            <div className="mt-8 rounded-[2rem] border border-dashed border-[var(--editable-border)] bg-white/70 p-10 text-center">
              <p className="editable-display text-2xl font-extrabold tracking-[-0.03em]">No matching posts found.</p>
