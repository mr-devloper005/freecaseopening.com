...
const cardBase = 'group block rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] shadow-[0_16px_38px_rgba(84,22,44,0.08)] transition duration-500 hover:-translate-y-1.5 hover:shadow-[0_24px_58px_rgba(84,22,44,0.16)]'

function archiveAdSlot(task: TaskKey): 'header' | 'sidebar' | 'in-feed' | 'article-bottom' | 'footer' {
  switch (task) {
    case 'article':
      return 'header'
    case 'profile':
  ...
  return (
    <EditableSiteShell>
      <main style={taskThemeStyle(task)} className="min-h-screen bg-[var(--tk-bg)] text-[var(--tk-text)]">
        <header className="relative overflow-hidden bg-[linear-gradient(180deg,#fb5a72_0%,#fb6371_58%,#ff7c6c_100%)] text-white">
          ...
          <div className="mx-auto max-w-[var(--editable-container)] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
            <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr] lg:items-end">
              <div>
                <div className="flex items-center gap-3 text-[11px] font-bold uppercase tracking-[0.32em] text-white/88">
                  <span>{theme.kicker}</span>
                  <span className="h-1 w-1 rounded-full bg-white/70" />
                  <span>{label}</span>
                </div>
                <h1 className="editable-display mt-6 max-w-4xl text-balance text-4xl font-extrabold leading-[1.04] tracking-[-0.04em] sm:text-5xl lg:text-[3.4rem]">
                ...
                {voice?.chips?.length ? (
                  <div className="mt-7 flex flex-wrap gap-3">
                    {voice.chips.map((chip) => (
                      <span key={chip} className="rounded-full bg-white/12 px-4 py-2 text-xs font-bold uppercase tracking-[0.18em] text-white backdrop-blur-sm">
                        {chip}
                      </span>
                    ))}
                  </div>
              ...
              <div className="rounded-[28px] bg-white/12 p-4 shadow-[0_18px_50px_rgba(154,3,30,0.18)] backdrop-blur-sm">
                <div className="rounded-[24px] bg-white p-4 sm:p-5">
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <p className="text-xs font-bold uppercase tracking-[0.24em] text-[var(--tk-accent)]">Live archive</p>
                      <p className="mt-2 text-sm text-[var(--tk-muted)]">
                        <span className="font-semibold text-[var(--tk-text)]">{posts.length}</span> {posts.length === 1 ? 'post' : 'posts'} in {categoryLabel}
                      </p>
                    </div>
                    <span className="rounded-full bg-[var(--tk-accent-soft)] px-4 py-2 text-xs font-bold uppercase tracking-[0.16em] text-[var(--tk-accent)]">
                      Page {page}
                    </span>
                  </div>
                  <form action={basePath} className="mt-4 flex flex-col gap-3 sm:flex-row">
                    <div className="relative flex-1">
                      <select
                        name="category"
                        defaultValue={category}
                        className="h-12 w-full appearance-none rounded-full border border-[var(--tk-line)] bg-[var(--tk-surface)] pl-4 pr-10 text-sm font-medium text-[var(--tk-text)] outline-none"
                    ...
                    <button className="inline-flex h-12 items-center justify-center rounded-full bg-[var(--slot4-brand-blue)] px-6 text-sm font-bold text-white transition hover:-translate-y-0.5">
                      Apply
                    </button>
                  </form>
                </div>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-12 sm:px-6 lg:px-8 lg:py-14">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.24em] text-[var(--tk-accent)]">{voice?.eyebrow || label}</p>
              <h2 className="editable-display mt-2 text-3xl font-extrabold tracking-[-0.03em] text-[var(--tk-text)]">
                Browse {categoryLabel.toLowerCase()}
              </h2>
            </div>
            <Link href={basePath} className="inline-flex items-center gap-2 rounded-full border border-[var(--tk-line)] bg-white px-5 py-3 text-sm font-bold text-[var(--tk-text)] transition hover:border-[var(--tk-accent)] hover:text-[var(--tk-accent)]">
          ...
          {posts.length ? (
            ...
            <div className="mx-auto max-w-xl rounded-[var(--tk-radius)] border border-dashed border-[var(--tk-line)] bg-[var(--tk-surface)] px-8 py-16 text-center shadow-[0_16px_36px_rgba(84,22,44,0.08)]">
          ...
          {posts.length ? (
            <nav className="mt-14 flex items-center justify-center gap-3 text-sm">
              ...
              <span className="rounded-full bg-[var(--tk-accent)] px-5 py-2.5 font-medium text-white">Page {page} of {pagination.totalPages || 1}</span>
  ...
  return (
    <div className={`mt-2.5 flex items-center gap-2 ${center ? 'justify-center' : ''}`}>
      <span className="inline-flex items-center gap-[3px]">
        {[0, 1, 2, 3, 4].map((i) => (
          <Star key={i} className={`h-4 w-4 ${i < filled ? 'fill-[var(--slot4-brand-2)] text-[var(--slot4-brand-2)]' : 'fill-[var(--tk-line)] text-[var(--tk-line)]'}`} />
        ))}
      </span>
  ...
  return (
    <Link href={href} className={`${cardBase} overflow-hidden`}>
      ...
      <div className="p-6 sm:p-7">
        ...
        <h2 className="editable-display mt-3 text-2xl font-extrabold leading-snug tracking-[-0.03em]">{post.title}</h2>
  ...
  return (
    <Link href={href} className={`${cardBase} flex items-center gap-5 p-5 sm:p-6`}>
      ...
      <div className="min-w-0 flex-1">
        <h2 className="editable-display truncate text-xl font-extrabold tracking-[-0.03em]">{post.title}</h2>
        <RatingLine post={post} />
...
function ClassifiedArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className={`${cardBase} flex flex-col p-6 sm:p-7`}>
      ...
      <h2 className="editable-display mt-5 text-xl font-extrabold leading-snug tracking-[-0.03em]">{post.title}</h2>
      <RatingLine post={post} />
...
function ImageArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  const image = getImage(post)
  return (
    <Link href={href} className="group mb-5 block break-inside-avoid overflow-hidden rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] shadow-[0_14px_36px_rgba(84,22,44,0.08)] transition duration-300 hover:-translate-y-1">
      <div className={`relative overflow-hidden ${index % 3 === 0 ? 'aspect-[3/4]' : 'aspect-[4/3]'}`}>
        ...
        <div className="absolute inset-x-0 bottom-0 p-5">
          <h2 className="editable-display line-clamp-2 text-lg font-extrabold leading-snug tracking-[-0.03em] text-white">{post.title}</h2>
  ...
  return (
    <Link href={href} className={`${cardBase} flex gap-4 p-6`}>
      ...
      <div className="min-w-0 flex-1">
        ...
        <h2 className="editable-display mt-1.5 text-lg font-extrabold leading-snug tracking-[-0.03em]">{post.title}</h2>
...
function PdfArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className={`${cardBase} flex flex-col p-6 sm:p-7`}>
      ...
      <h2 className="editable-display mt-6 text-xl font-extrabold leading-snug tracking-[-0.03em]">{post.title}</h2>
      <RatingLine post={post} />
      <p className="mt-3 line-clamp-3 flex-1 text-sm leading-7 text-[var(--tk-muted)]">{getSummary(post)}</p>
      <span className="mt-6 inline-flex items-center gap-1.5 text-sm font-bold text-[var(--tk-accent)]">Open document <Download className="h-4 w-4" /></span>
    </Link>
  )
}
...
function ProfileArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className={`${cardBase} flex flex-col items-center p-7 text-center`}>
      ...
      <h2 className="editable-display mt-5 text-lg font-extrabold tracking-[-0.03em]">{post.title}</h2>
